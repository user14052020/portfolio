...
class StyleFacetKnowledgeProjectorTests(unittest.TestCase):
    def test_projector_builds_documents_chunks_and_runtime_cards_with_versions(self) -> None:
        ...
        projector = DefaultStyleFacetKnowledgeProjector()
        projection = projector.project_from_source(
            source=StyleFacetProjectionSource(
                ...
                traits=[trait],
                relations=[relation],
                knowledge_facet=knowledge_facet,
                visual_facet=visual_facet,
                fashion_facet=fashion_facet,
                image_facet=image_facet,
                relation_facet=relation_facet,
                presentation_facet=presentation_facet,
            )
        )

        self.assertEqual(projection.provider_code, "style_ingestion")
        self.assertEqual(projection.facet_version, "2026-04")
        self.assertEqual(projection.parser_version, "1")
        self.assertEqual(projection.normalizer_version, "1")
        self.assertEqual(len(projection.documents), 1)
        self.assertGreaterEqual(len(projection.chunks), 8)
        self.assertGreaterEqual(len(projection.cards), 9)
        self.assertEqual(projection.primary_runtime_card().knowledge_type, KnowledgeType.STYLE_CATALOG)
        self.assertIn(KnowledgeType.STYLE_VISUAL_LANGUAGE, [card.knowledge_type for card in projection.cards])
        self.assertIn(KnowledgeType.STYLE_IMAGE_COMPOSITION, [card.knowledge_type for card in projection.cards])
        self.assertIn(KnowledgeType.STYLE_RELATION_CONTEXT, [card.knowledge_type for card in projection.cards])
        self.assertIn("projection_version", projection.documents[0].metadata)
        self.assertEqual(projection.documents[0].metadata["facet_version"], "2026-04")
