...
export function GenerationStyleExplanation({
  ...
  return (
    <div className="rounded-[24px] border border-[#e7d7bf] bg-[#f8f4ed]/95 px-4 py-3 shadow-[var(--shadow-soft-sm)]">
      <p className="text-[11px] font-medium uppercase tracking-[0.18em] text-[var(--text-muted)]">
        {locale === "ru" ? "О стиле" : "About the style"}
      </p>
      {title ? <p className="mt-2 text-sm font-medium text-[var(--text-primary)]">{title}</p> : null}
      {lines.length > 0 ? (
