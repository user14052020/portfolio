...
from app.application.reasoning.services.locale_text_policy import LocaleTextPolicy
...
class DefaultVoiceLayerComposer:
    def __init__(
        ...
        *,
        ...
        locale_text_policy: LocaleTextPolicy | None = None,
    ) -> None:
        ...
        self._locale_text_policy = locale_text_policy or LocaleTextPolicy()
        ...
        self._voice_prompt_builder = voice_prompt_builder or DefaultVoicePromptBuilder(
            voice_runtime_flags=self._voice_runtime_flags,
            voice_runtime_settings_provider=voice_runtime_settings_provider,
            locale_text_policy=self._locale_text_policy,
        )
    ...
    async def compose(
        ...
        if self._enable_model_composition and self._voice_composition_client is not None:
            try:
                ...
                validation = self._locale_text_policy.validate_reply(
                    text=" ".join(
                        item
                        for item in (draft.final_text, draft.cta_text or "")
                        if item
                    ),
                    locale=context.locale,
                )
                if not validation.is_valid:
                    raise ValueError(f"voice composition locale validation failed: {validation.reason}")
        ...
        final_text, request_echo_removed = self._strip_leading_request_echo(
            text=final_text,
        ...
        observability = {
            **dict(voice_prompt.observability),
            ...
            "voice_locale_guard_used": bool(llm_error),
            "voice_request_echo_removed": request_echo_removed,
        }
        return StyledAnswer(
            text=final_text,
            tone_profile=tone_profile,
    ...
    def _safe_base_text(self, *, reasoning_output: FashionReasoningOutput, locale: str) -> str:
        base_text = self._base_text(reasoning_output=reasoning_output).strip()
        validation = self._locale_text_policy.validate_reply(text=base_text, locale=locale)
        if validation.is_valid:
            return base_text

        brief = reasoning_output.fashion_brief
        items: list[str] = []
        palette: list[str] = []
        if brief is not None:
            ...
            palette = list(brief.palette[:3])

        if self._is_russian_locale(locale):
            item_text = ", ".join(item for item in items[:5] if item) or "цельный образ"
            palette_text = ", ".join(item for item in palette if item)
            if palette_text:
                return f"Собрал образ под задачу: {item_text}. Палитра: {palette_text}."
            return f"Собрал образ под задачу: {item_text}."

        item_text = ", ".join(item for item in items[:5] if item) or "one coherent outfit"
        palette_text = ", ".join(item for item in palette if item)
        if palette_text:
            return f"I built the outfit around: {item_text}. Palette: {palette_text}."
        return f"I built the outfit around: {item_text}."

    def _strip_leading_request_echo(
        self,
        *,
        text: str,
        ...
        normalized_request = " ".join((user_request or "").split())
        if not normalized_text or not normalized_request:
            return text, False

        prefixes = (
            normalized_request,
            f"For {normalized_request}",
            f"for {normalized_request}",
            f"Для {normalized_request}",
            f"По запросу {normalized_request}",
            f"По запросу: {normalized_request}",
        )
        lowered_text = normalized_text.lower()
        for prefix in prefixes:
            if not lowered_text.startswith(prefix.lower()):
                continue
            remainder = normalized_text[len(prefix) :].lstrip(" \t\r\n,.:;!?-–—")
            if remainder:
                return self._uppercase_first(remainder), True
        return text, False
    ...
    def _uppercase_first(self, text: str) -> str:
        if not text:
            return text
