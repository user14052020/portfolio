...
class VoiceLayerComposerTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_composer_strips_leading_user_request_echo_from_model_answer(self) -> None:
        user_request = (
            "Собираем мужской стиль. Хочу более выразительный стиль. "
            "Ближе собранный и структурный силуэт. Нужна нейтральная палитра."
        )
        composer = DefaultVoiceLayerComposer(
            voice_tone_policy=DefaultVoiceTonePolicy(),
            voice_prompt_builder=DefaultVoicePromptBuilder(),
            voice_composition_client=FakeVoiceCompositionClient(
                VoiceCompositionDraft(
                    final_text=f"For {user_request}, use structured charcoal tailoring with a cleaner shoulder line.",
                    cta_text=None,
                    provider_model="qwen35b:latest",
                    raw_content='{"final_text":"echoed"}',
                )
            ),
            enable_model_composition=True,
        )
        ...
        answer = await composer.compose(
            FashionReasoningOutput(
                ...
                text_response="Use structured charcoal tailoring.",
                can_offer_visualization=True,
            ),
            VoiceContext(
                mode="style_exploration",
                response_type="brief_ready_for_generation",
        ...
        self.assertEqual(
            answer.text,
            "Use structured charcoal tailoring with a cleaner shoulder line.",
        )
        self.assertNotIn(user_request, answer.text)
        self.assertTrue(answer.observability["voice_request_echo_removed"])
    ...
    async def test_composer_rejects_model_reply_with_forbidden_cjk_script(self) -> None:
        composer = DefaultVoiceLayerComposer(
            voice_tone_policy=DefaultVoiceTonePolicy(),
            voice_prompt_builder=DefaultVoicePromptBuilder(),
            voice_composition_client=FakeVoiceCompositionClient(
                VoiceCompositionDraft(
                    final_text="Для этого свидания я бы рекомендовал轻柔的春日午后.",
                    cta_text=None,
                    provider_model="qwen35b:latest",
                    raw_content='{"final_text":"bad"}',
                )
            ),
            enable_model_composition=True,
        )
        ...
        answer = await composer.compose(
            FashionReasoningOutput(
                response_type="generation_ready",
                text_response="Для этого свидания я бы рекомендовал轻柔的春日午后.",
                fashion_brief=FashionBrief(
                    brief_mode="occasion_outfit",
                    style_identity="date",
                    garment_list=["льняная рубашка", "лёгкие брюки"],
                    footwear=["лоферы"],
                    accessories=["тонкий ремень"],
                    palette=["молочный", "тёмно-синий"],
                ),
                can_offer_visualization=True,
        ...
        self.assertFalse(any("\u4e00" <= char <= "\u9fff" for char in answer.text))
        self.assertIn("льняная рубашка", answer.text)
        self.assertIn("молочный", answer.text)
        self.assertFalse(answer.observability["voice_llm_used"])
        self.assertTrue(answer.observability["voice_llm_fallback_used"])
