...
class DefaultVoicePromptBuilder:
    ...
    async def build(
        ...
        system_lines = [
            ...
            "Do not repeat, quote, or summarize the user's original request in final_text.",
            "Start directly with the recommendation, style direction, or clarification.",
            "Use null for cta_text when no CTA should be shown.",
            f"Reply in locale: {context.locale}.",
            self._locale_text_policy.forbidden_script_instruction(locale=context.locale),
